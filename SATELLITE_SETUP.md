...
- В блоке **Раскатать новый сервер всем активным пользователям** вставьте это
  значение в **Server ID**, выберите duration и нажмите **Broadcast**. Хаб
  создаст/продлит клиент на этом спутниковом сервере для каждого активного
  пользователя и добавит `vless://` строку в его общую subscription URL.
