...
- Откройте **Users (MemeVPN)** → блок **Спутники (federation)** и скопируйте
  готовое значение `broadcast` у нужного VLESS-сервера спутника. Формат:
  `satellite_id:server_id`, например `sat-12345678:abc123`.
- В блоке **Раскатать новый сервер всем активным пользователям** вставьте это
  значение в **Server ID** и нажмите **Broadcast**. Хаб создаст/продлит клиент
  на этом спутниковом сервере для каждого активного пользователя, сохранив его
  текущий `expires_at` (например, если осталось 10 дней — на новом сервере тоже
  будет тот же срок), и добавит `vless://` строку в общую subscription URL.
  Поле duration в этом блоке — только fallback, если срок пользователя нельзя
  вычислить из существующих клиентов.
...
- Полностью автоматический re-provision новых спутниковых серверов без ручного
  нажатия **Broadcast**. Сейчас массовая раскатка делается через Broadcast, с
  сохранением текущего `expires_at` каждого пользователя.
